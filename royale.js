// clash.js — DPI-safe canvas, king towers, trippi troppis stats restored to normal hit/move speed,
// improved targeting so troops prefer nearest enemy troops (unless they specifically target towers)
// and show unit names on each model
(function(){
  ...
  canvas.addEventListener('pointerdown', (ev)=>{
    ...
    if(def.troopType !== 'spell' && y <= H/2){ selectedCard = null; renderHand(); return; }

    // Clamp spawn inside arena margins so units don't spawn off-screen
    const margin = 40;
    const spawnX = Math.max(margin, Math.min(W - margin, x));
    const spawnY = (def.troopType === 'spell') ? Math.max(margin, Math.min(H - margin, y)) : Math.max(H/2 + 20, Math.min(H - margin, y));
  ...
  // Improved findTarget: prefer nearest enemy troop (even if out of range), unless unit specifically targets towers only.
  function findTarget(u){
    // If this unit is tower-only, skip troop selection
    if(u.targets === 'tower'){
      // pick nearest enemy tower
  ...
  function draw(){
    ...
    // units (draw name above each unit for easier testing)
    ...
    for(const u of units){
      ...
      // name label
      ctx.fillStyle = '#fff';
      ctx.fillText(u.name, u.x, u.y - u.size - 6);
